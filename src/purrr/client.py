...
class CachedPrefectClient:
    ...
    async def get_deployment_by_id(self, deployment_id: UUID) -> DeploymentResponse | None:
        try:
...
class RunsClient:
    ...
    def upsert(self, flow_run: FlowRun):
    ...
    def upsert_many(self, flow_runs: list[FlowRun]):
        for flow_run in flow_runs:
            self.upsert(flow_run)
...
class LogsClient:
    ...
    def upsert_many(self, logs: list[Log]):
        for log in logs:
            self.upsert(log)

    def read_by_flow_run(self, flow_run_id: UUID | str) -> list[dict]:
...
class LocalDuckDB:
    def __init__(self, db_path: str = "duckdb.db"):
        ...
        self.logs = LogsClient(self.db)
        self.runs = RunsClient(self.db)
